...
function M.new(data, dir)
  ...
  for i = 1, #tilesets do
    if type( tilesets[i].tiles ) == "table" then
      ...
      for _, t in pairs( tilesets[i].tiles ) do
        ...
        if t.properties then --- MY OWN CODE
          local temp = {}
          for key,value in pairs(t.properties) do
            temp[key] = value
          end
          _tileData[n] = temp
        end
  ...
  local function gidLookup(gid)
    ...
    gid = clearbit(gid, FlippedDiagonallyFlag) --------------------- gid ist hier noch definiert.
    ...
    for i = 1, #tilesets do
      ...
      if gid >= firstgid and gid <= lastgid then
        if tileset.image then -- spritesheet
          --print(tileset.image, ":is image tileset") --DEBUG
          local sequenceData
          ...
          for t = 1, #tileset.tiles do
            local tile = tileset.tiles[t]
            return tile.image, flip -- temporäre Lösung
    ...
    if layer.type == "tilelayer" then
      ...
      for ty=0, data.height-1 do
        for tx=0, data.width-1 do
          ...
          if gid then -- Nur die Tiles, die eine Information enthalten, sprich nicht Luft sind, haben einen gid.
            local image
            if animation then
              --print("Animating:", gid)
